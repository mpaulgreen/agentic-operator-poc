...
type RedisClusterSpec struct {
	...
	// TLS defines the optional TLS/SSL configuration for Redis connections.
	// +optional
	TLS *TLSSpec `json:"tls,omitempty"`

	// MaxMemory defines the maximum memory Redis can use (e.g., "256Mi", "1Gi").
	// +optional
...
// TLSSpec defines TLS/SSL configuration for Redis connections.
type TLSSpec struct {
	// Enabled indicates whether TLS is active for Redis connections.
	// +kubebuilder:default=false
	Enabled bool `json:"enabled,omitempty"`

	// SecretName is the name of a Secret containing tls.crt and tls.key.
	// Required when enabled is true and certManager is false.
	// +optional
	SecretName string `json:"secretName,omitempty"`

	// CertManager indicates whether cert-manager should automatically provision certificates.
	// +kubebuilder:default=false
...
type RedisClusterStatus struct {
	...
	// TLSEnabled indicates whether TLS is currently active on the Redis cluster.
