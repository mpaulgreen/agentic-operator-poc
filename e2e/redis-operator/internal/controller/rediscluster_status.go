...
func (r *RedisClusterReconciler) updateStatus(ctx context.Context, cr *cachev1alpha1.RedisCluster) error {
	...
	cr.Status.MasterEndpoint = fmt.Sprintf("%s-client.%s.svc.cluster.local:6379", cr.Name, cr.Namespace)
...
func (r *RedisClusterReconciler) updatePhase(cr *cachev1alpha1.RedisCluster, sts *appsv1.StatefulSet) {
	...
	case ready == 0:
		...
		setUnavailableCondition(cr, "NoPodsReady", "No Redis pods are ready")
	...
	case ready < desired:
		...
		setAvailableCondition(cr, "PartiallyAvailable",
			fmt.Sprintf("RedisCluster has %d/%d ready replicas", ready, desired))
