...
func setCondition(cr *backupv1beta1.MongoBackupPolicy, conditionType string, status metav1.ConditionStatus, reason, message string) {
	now := metav1.Now()
...
func setAvailableCondition(cr *backupv1beta1.MongoBackupPolicy, reason, message string) {
	setCondition(cr, ConditionAvailable, metav1.ConditionTrue, reason, message)
}

func setDegradedCondition(cr *backupv1beta1.MongoBackupPolicy, reason, message string) {
	setCondition(cr, ConditionDegraded, metav1.ConditionTrue, reason, message)
}

func clearDegradedCondition(cr *backupv1beta1.MongoBackupPolicy, reason, message string) {
